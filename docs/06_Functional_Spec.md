...
### 동작
- Featured Projects는 `isFeatured=true` AND `isPublic=true` 조건을 기본으로 노출
...
### 리스트
- 컬럼: Title / Visibility(Public) / Featured / UpdatedAt
- 기능: 검색(Title), 필터(Public only), 정렬(UpdatedAt)
...
- isFeatured=true면 isPublic도 true여야 함(규칙)
...
- title (필수)
- company (선택)
- period (선택: YYYY-MM ~ YYYY-MM 또는 텍스트)
- role (선택)
